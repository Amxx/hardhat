...
impl<LogT: serde::Serialize> Serialize for TypedReceipt<LogT> {
        ...
        S: Serializer,
    {
        ...
        match &self.data {
            ...
            | TypedReceiptData::Eip5806 { status } => {
...
impl<LogT> TypedReceipt<LogT> {
    ...
    pub fn status_code(&self) -> Option<u8> {
        match &self.data {
            ...
            | TypedReceiptData::Eip1559 { status }
            | TypedReceiptData::Eip4844 { status }
            | TypedReceiptData::Eip5806 { status } => Some(*status),
        }
    }
                ...
                let data = if let Some(state_root) = state_root {
                    ...
                    if let Some(transaction_type) = transaction_type {
                        match transaction_type.as_str() {
                            ...
                            "0x4" => TypedReceiptData::Eip5806 { status },
    ...
    fn decode(buf: &mut &[u8]) -> alloy_rlp::Result<Self> {
            ...
            LogT: Decodable,
        {
            ...
            let data = match id {
                ...
                Some(4) => TypedReceiptData::Eip5806 {
                    status: normalize_status(u8::decode(buf)?),
                },
    ...
    LogT: Encodable,
{
    fn encode(&self, out: &mut dyn BufMut) {
        let id = match &self.data {
            ...
            TypedReceiptData::Eip5806 { .. } => Some(4u8),
        };
...
mod tests {
    ...
    impl_typed_receipt_tests! {
        ...
        eip5806 => TypedReceiptData::Eip5806 { status: 0 },
    }
