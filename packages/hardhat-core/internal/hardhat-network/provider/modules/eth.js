...
class EthModule extends base_1.Base {
    ...
    async _callAction(rpcCall, blockTag, stateOverrideSet) {
        ...
        if (error !== undefined && this._throwOnCallFailures) {
            const callReturnData = Buffer.from(trace?.returnData ?? []).toString("hex");
    ...
    async _getTransactionByHashAction(hash) {
        ...
        const index = block.transactions.findIndex((btx) => (0, ethereumjs_util_1.equalsBytes)(btx.hash(), hash));
    ...
    async _sendRawTransactionAction(rawTx) {
        ...
        if (tx instanceof ethereumjs_tx_1.LegacyTransaction) {
    ...
    async _rpcTransactionRequestToNodeTransactionParams(rpcTx) {
        const baseParams = {
            ...
            data: rpcTx.data !== undefined ? rpcTx.data : (0, ethereumjs_util_1.toBytes)([]),
    ...
    async _sendTransactionAndReturnHash(tx) {
        ...
        if (trace.error !== undefined && this._throwOnTransactionFailures) {
            const e = trace.error;
            const returnData = Buffer.from(trace.trace?.returnData ?? []).toString("hex");
    ...
    _getTransactionResultAndIndex(tx, results) {
        for (const result of results) {
            ...
            for (let i = 0; i < transactions.length; i++) {
                ...
                if ((0, ethereumjs_util_1.equalsBytes)(blockTx.hash(), tx.hash())) {
                    return [result, i];
                }
            }
        }
